...
ga.monitornew <- function (object, digits = getOption("digits"), ...) 
{
  ...
  #  save(object, file = "GA_intermediate_severe_TestX5.Rdata")
...
GA.severe.X5 <- ga(type = "real-valued", fitness = obj.fun.par, dm.data = dm.realization, test.total = dat.agg$Z*5, goal = "severe", repi = repi, lower = rep(0, 21), upper = rep(1, 21),  keepBest = TRUE, popSize = 50, maxiter = 100, monitor = ga.monitornew, run = 100, pmutation = 0.05, suggestions = GA.sugguestion)

save(GA.severe.X5, file = "Result\\Optimization\\GA_severe_TestX5.Rdata")
